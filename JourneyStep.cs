...
namespace Journeys
{
    ...
    public class JourneyStep
    {
        ...
        private List<Waypoint> m_route;         // really not needed after debugging, only needed to redraw the mesh from scratch (which is not needed)

        public Mesh[] m_RouteMeshes;        // should be private, just public for debug
        private Color m_routemeshColor;
        ...
        public void KillRouteMeshes()
        {
            lock (jstepLock)
            {
                ...
                for (int idx = 0; idx < m_RouteMeshes.Length; idx++)
                {
                    ...
                    if (mesh != null) UnityEngine.Object.Destroy(mesh);  // this might not really be needed, GC should take care of it, but I follow PV usage
                }
                m_RouteMeshes = null;
            }
        }
        ...
        public JourneyStep(List<Waypoint> route, ushort citizenID, LineColorPair lineColorPair, bool endJourney, bool show = true)
        {
            ...
            m_route.AddRange(route);
                ...
                m_routemeshColor = lineColorPair.m_lineColor;
...
//            SetRouteMesh(route, endJourney, lineColorPair.m_lineColor, 4f, 5f);
        ...
        public void SetRouteMesh()
        {
            lock (jstepLock)
            {
                //m_endJourney = endJourney;
                //m_routemeshColor = lineColor;
                //m_routemeshHalfwidth = halfwidth;
                //m_routemeshHalfheight = halfheight;
                if (m_route == null)
                {
                    Debug.Log("JV: SetRouteMesh called with empty m_route");
                    return;
                }
                else
                {
                    if (NeedsMesh)
                    {
                        Mesh[] meshes = CreateMesh(m_route, m_endJourney, m_routemeshHalfwidth, m_routemeshHalfheight);
                        m_RouteMeshes = new Mesh[meshes.Length];
                        m_RouteMeshes = meshes;
        ...
        public void DrawTheMeshes(RenderManager.CameraInfo cameraInfo, Material material, bool heatmap = false, int denominator = 0, bool ignoreHidden = true)
        {
            lock (jstepLock)
            {
                ...
                if (m_RouteMeshes == null)
                    return;
                material.color = m_routemeshColor;
        ...
        private static Mesh[] CreateMesh(List<Waypoint> route, bool endJourney, float halfwidth, float halfheight)
        {
            if (route.Count == 0)
            {
                Debug.Log("JV: createmesh called with empty route");
                return null;
            }
            ...
            for (int index = 0; index < meshDataArrayLength; ++index)
            {
                ...
                newmesh[index].vertices = meshDataArray[index].m_vertices;
                newmesh[index].normals = meshDataArray[index].m_normals;
                newmesh[index].tangents = meshDataArray[index].m_tangents;
                newmesh[index].uv = meshDataArray[index].m_uvs;
                newmesh[index].uv2 = meshDataArray[index].m_uvs2;
                newmesh[index].colors32 = meshDataArray[index].m_colors;
                newmesh[index].triangles = meshDataArray[index].m_triangles;
                newmesh[index].bounds = meshDataArray[index].m_bounds;
            }
            return newmesh;
        }
