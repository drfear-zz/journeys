...
namespace Journeys
{
    ...
    public class JourneyVisualizer : MonoBehaviour
    {
        ...
        private Dictionary<ushort, JourneySegmentData> m_gridData;
        ...
        public void Init()
        {
            ...
            m_journeysCount = 0;
            m_gridData = new Dictionary<ushort, JourneySegmentData>();
...
private void StepJourney(Journey journey)
        ...
        private bool RefreshJourney(Journey journey)
        {
            ...
            // note that we already converted public transport segments to map segments when we created the Journey from the path,
            // so we only have to deal with map segments with "normal" meshes here
            ...
            for (int sjIndex = 0; sjIndex < journey.m_subJourneys.Count; ++sjIndex)
            {
                ...
                float lengthScale = Mathf.Ceil(totalLength / 64f) / totalLength * journey.m_arrowOffset;
    ...
    public static bool CalculateSubJourneySegmentCount(
            ...
            for (int stepIndex = 0; stepIndex < subJourney.m_journeySteps.Count; ++stepIndex)
            {
                ...
                else
                {
                    ...
                    if ((double)distanceClosestToPrevious > 1.0)
                    {
                        ...
                        //TransportLine.FillPathSegment(bezier, data[index2].m_meshData, curves, data[index2].m_pathSegmentIndex, curveIndex, currentLength * lengthScale * 1.1f, overallLength * lengthScale * 1.1f, 4f, !ignoreY ? 10f : 40f, ignoreY);
                        // Debug.Log("curveindex: " + curveIndex + ", currentLength: " + currentLength + ", overallLength: " + overallLength + ", lengthScale: " + lengthScale);
                    ...
                    if ((double)distanceAlongThisStep > 1.0)
                    {
                        ...
                        //float scaledLength = overallLength * lengthScale;
                        //float threeqLength = scaledLength * 0.25f;
                        //TransportLine.FillPathSegment(subCurve, data[index2].m_meshData, curves, data[index2].m_pathSegmentIndex, curveIndex, 10f, 11f, 4f, !ignoreY ? 5f : 20f, ignoreY);
                        if (curveOffsets != null)
                            curveOffsets[curveIndex] = new Vector2(currentLength * lengthScale, overallLength * lengthScale);
                        ++data[index2].m_pathSegmentIndex;
                        ++curveIndex;
        ...
        // this is also the point at which we add the journey segments to the gridData
            ...
            try
            {
                ...
                for (int index = 0; index < m_removeJourneys.m_size; ++index)
                {
                    ...
                    for (int sjIndex = 0; sjIndex < thisJourney.m_subJourneys.Count; ++sjIndex)
                    {
                        ...
                        ushort lineID = thisSubJourney.m_lineColorPair.m_lineID;
                        foreach (Journey.SubJourney.JourneyStep jStep in thisSubJourney.m_journeySteps)
                        {
                            ReduceGridData(jStep.m_segment, cit, lineID);
                        }
                    }
