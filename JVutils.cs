...
namespace Journeys
{
    public static class JVutils
    {
        ...
        // I considered combining meshfromdata with a transform of width, but nothing is really gained except slowing down
        // the by-far-most-common case that you do not need to transform

        //public static Mesh[] MeshFromData(RenderGroup.MeshData[] data, float currentHalfwidth, float newHalfwidth)
        //{
        //    int dataLength = data.Length;
        //    Mesh[] newmesh = new Mesh[dataLength];
        //    for (int index = 0; index < dataLength; ++index)
        ...
        //        float weight1 = (width - delta) / width;
        //        float weight2 = delta / width;
        //        foreach (Mesh mesh in newmesh)
        //        {
        ...
        //                    newVertices[absIdx1] = (weight1 * v1) + (weight2 * v2);
        //                    newVertices[absIdx2] = (weight2 * v1) + (weight1 * v2);
        ...
        public static Color ColourHeat(float numerator, float denominator = 0, float discreteCats = 0)
        {
            if (denominator == 0)
            ...
            return Color.HSVToRGB(H: (1f - ratio) / 2, S: 0.8f, V: 0.8f);
        }

        public static float HalfWidthHeat(float numerator, float denominator = 0, float discreteCats = 0)
        {
            if (denominator == 0)
                denominator = Singleton<JourneyVisualizer>.instance.m_journeysCount;
            float ratio = numerator / denominator;
            if (discreteCats == 1)
                ratio = 0.5f;                                                   // artificial set halfwidth to 4 rather than 8 if all lines same halfwidth
            else
                if (discreteCats > 0)
                    ratio = Mathf.Ceil(ratio * discreteCats) / discreteCats;
            return Mathf.Ceil(ratio * 8);                                           // hardcoded max halfwidth as 8, and discretized to integer
        }
